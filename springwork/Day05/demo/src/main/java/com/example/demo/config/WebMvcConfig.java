...
public class WebMvcConfig implements WebMvcConfigurer{
	...
	//스프링은 기본적으로 MVC동작을 설명해주지만, 때로는 프로젝트 요구사항에 맞게 특정 기능을 추가하거나
	//수정해야 할 WebMvcConfigurer를 구현하여 원하는 설정을 적용할 수 있다.
	//주요 기능
	...
	//CORS 설정  = addCors 오버라이딩
		@Override
		public void addCorsMappings(CorsRegistry registry) {
			//↓ 모든 경로에 대해 CORS 설정을 적용
			registry.addMapping("/**")
			//↓ React 애플리케이션이 실행되는 도메인(출처 = oirigins)에서 오는 요청을 허용
			.allowedOrigins("http://localhost:3000")
			//↓ HTTP 메서드(GET,POST,PUT,DELETE)를 허용
			.allowedMethods("GET","POST","PUT","DELETE")
			//↓ 모든 헤더를 허용하겠다.
			.allowedHeaders("*")
			//↓ 쿠키가 인증 정보를 포함한 요청을 허용
			.allowCredentials(true)
			//↓ 브라우저가 서버로부터 받은 응답을 일정 시간동안 저장해두고, 그 시간 냉네 동일한 요청이 있을 경우
